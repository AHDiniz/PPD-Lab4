...
def callback_solution(ch, method, properties, body):
    ...
    pika_connection = return_connection()
    channel = pika_connection.channel
    channel.basic_publish(
        exchange=voting_channel,  routing_key='', body=json.dumps(voting, indent=4, cls=CustomEncoder))
    channel.close()
    pika_connection.connection.close()
    ...
    if (len(voting) == max_clients):
        ...
        if (sum(elem.valid for elem in voting) > max_clients / 2):
            ...
            if (max(election, key=lambda x: x['vote'])['id'] == local_id):
                ...
                channel = pika_connection.channel
                channel.basic_publish(
                    exchange=challenge_channel, routing_key='', body=json.dumps(transaction, indent=4, cls=CustomEncoder))
                channel.close()
                pika_connection.connection.close()
        else:
...
class ExampleConsumer(object):
    ...
    def on_channel_open(self, channel):
        ...
        self.setup_exchange(init_channel)
        self.setup_exchange(election_channel)
        self.setup_exchange(challenge_channel)
        self.setup_exchange(solution_channel)
        self.setup_exchange(voting_channel)
